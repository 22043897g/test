...
const {getData} = require("../utils/tngateway");
...
//Clear location location_id.json
...
//每天0点执行一次 0 0/1 * * * ? conf.beforeDawn
schedule.scheduleJob(conf.beforeDawn, async () => {
    try {
...
//中午12点执行一次
schedule.scheduleJob(conf.noon, async () => {
    try {
        let filename = `${conf.rootPath}/public/location_id.json`
        let jsonFile = fs.readFileSync(filename, 'utf8')
        const leanwells = JSON.parse(jsonFile);
        await autoGetProduct(leanwells)

        //中午12点执行后清空location_id.json
        clearJsonArray()
    } catch (error) {
        console.error(error);
    }
